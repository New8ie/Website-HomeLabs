...
export default function BlogManager({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-2">
      {featuredPost && (
        <div className="lg:col-span-4 py-4 px-2  mb-8">
          <a href={`/blog/${featuredPost.slug}`} className="block w-full">
            <div
              className="relative bg-zinc-800/80 backdrop-blur-sm shadow-2xl rounded-2xl border border-zinc-700 mx-auto
                        h-80 w-full flex items-center justify-center p-4"
            >
              <div className="hidden">
                <h2 className="text-lg font-bold text-white font-cinzel leading-tight text-center truncate">
                  {featuredPost.data.title}
                </h2>
              </div>
              <div className="absolute opacity-100 w-full h-full">
                <div className="relative py-4 px-4 w-full h-full">
                  <img
                    src={
